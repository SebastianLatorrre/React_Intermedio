...
//       <BootstrapDialog onClose={handleClose} aria-labelledby="customized-dialog-title" sx={{color:(theme.palette.background.default)}} open={open} maxWidth="md">
...
import RemoveIcon from '@mui/icons-material/Remove';
import { useNavigate } from 'react-router-dom';
import { useCart } from '../Car/CartContext'; // Ajusta la ruta según tu estructura
import { theme } from '../../style/theme';
...
const CardModalInfo = ({ product }: CardModalInfoProps) => {
  ...
  const handleAddToCart = () => {
    ...
    addToCart(itemToAdd);
    handleClose();
    // Puedes añadir una confirmación o navegación adicional aquí si es necesario
  };

  const priceCol = Number(product.price) * 3936;
  const discount = 15; // Pendiente de implementación dinámica
  const priceDiscount = priceCol * (discount / 100);
  const COP = new Intl.NumberFormat('en-DE');
